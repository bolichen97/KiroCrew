...
def _visible_chat_slots() -> tuple[list[dict], str | None]:
    """The live sessions these tools may see, private ones removed.

    A slot whose ``memory_mode`` is not ``persistent`` — incognito or
    temporary — is a session the user chose to keep out of the record. The
    endpoint returns it like any other, so the filter belongs here, at the ONE
    place the list enters this server: both the tree render and the session
    resolver read the result, so neither can expose a private title or key, and
    neither can file one into a folder. An absent field reads as persistent,
    matching the slot default.
    """
    rows, err = _get_rows("/api/chat/slots")
    if err:
        return [], err
    return [r for r in rows if str(r.get("memory_mode") or "persistent") == "persistent"], None
