...
| `agent.subagent_max_concurrent_startups` | How many subagents may be in startup at once (admitted, but no runtime process, first provider stream or turn yet); further spawns wait in the spawn queue. `0` derives `max(2 × session_start_concurrency, ceil(cap / 4))`, never above the running cap | `0` |
