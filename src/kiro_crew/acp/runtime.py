...
class AcpRuntime:
    ...
    async def create_session(
        ...
        params = build_session_new_params(
            ...
            # Adapter-only seam: _kas_custom_agents returns None on the kiro
            # backend, so the kiro construction path gains no conditional, no
            # new required argument, and no new failure mode (harness-parity
            # H13). The KAS decision lives inside the seam, not here.
            kas_custom_agents=await self._kas_custom_agents(agent or self._agent),
        )
