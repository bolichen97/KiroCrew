...
def _parse_token_output(stdout_b: bytes, stderr_b: bytes) -> dict[str, Any]:
    ...
    if kind == "error":
        # This path carries no token; the message is kiro-cli's own (e.g. "not
        # logged in"). Redact defensively before it leaves the module.
        raw = str(data.get("message") or "authentication failed")
        redacted, _ = redact_credentials(raw)
        raise KasAuthCallbackError(redacted)
