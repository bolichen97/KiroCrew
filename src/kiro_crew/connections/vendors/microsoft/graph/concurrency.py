...
============================================
The auth chain is W01's: a ``BindingCustodyGate`` refuses a call whose trusted
``TrustedHandleView`` is not the binding it was composed for BEFORE the locator,
the store or the vault is touched, and a live ``BindingStore`` fences the binding
against its CURRENT record (revoked / stale / forged generation / swapped
identity all refused) and takes the ``secret_ref`` from that record -- never a
provider slug, never a caller copy -- before the transport reveals the credential
into ``Authorization`` for exactly one send. Routing every hop through
``execute`` is what makes that hold; a hop that called a sender directly would
attach no credential and pass no gate. Correlation is the executor's own call:
each :class:`ExecutionOutcome` this orchestration reads is ``execute``'s return
for the request the seat produced. ``request-id`` / ``client-request-id`` are
never read and are no auth proof.
...
def build_graph_write_dispatch(
    *,
    ...
    CUSTODY IS ``gate`` + ``store``, not a fixed secret ref. W01's composition
    (PRODUCTION_SCHEMA_VERSION 4) resolves the credential per call in two trusted
    pieces this entry passes straight through:

    * ``gate`` -- a
      :class:`~kiro_crew.connections.control_plane.production.BindingCustodyGate`,
      composed for ONE binding. On each call it matches the call's trusted
      ``TrustedHandleView`` and yields the binding to fence, refusing (before the
      locator, the store or the vault is touched) any call whose identity is not
      the one it holds custody for. It selects no secret and derives no vault
      name.
    * ``store`` -- a live
      :class:`~kiro_crew.connections.control_plane.lifecycle.BindingStore`, the
      TRUSTED PERSISTENT source. W01's transport calls its ``select_secret`` to
      fence the binding against the store's CURRENT record (revoked / stale /
      forged generation / swapped identity all refused) and take the
      ``secret_ref`` from that record -- never from a provider slug, never from a
      caller copy. A revoke or rotation takes effect on the next call with no
      cache.
    ...
    ``handle`` / ``gate`` / ``store`` / ``vault`` / ``permitted`` / ``layers`` are
    W01 types passed in by the composing caller (kept as ``Any`` here so this
    vendor module does not re-import W01's whole type surface). ``now`` is for a
    deterministic test only.

    NOTE: W01's auth chain (L03 auth-code, L04 rotation fencing, and the
    principal->binding resolver that :class:`BindingCustodyGate` names as a
    remaining gap -- one gate serves one binding) is NOT complete and carries
    reported gaps, and W01's own fresh install is still being corrected; this
    entry rides W01's chain but does not make it complete or safe on its own.
    """
