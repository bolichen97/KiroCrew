...
* ``EXECUTOR_SCHEMA_VERSION == 2`` -- the executor envelope, transport-callable
  contract (``trusted_view`` is passed) and paging surface this drives.
* ``PRODUCTION_SCHEMA_VERSION == 2`` -- ``build_production_transport`` taking a
  ``BindingSecretSelector`` (per-call custody), not a fixed ``secret_ref``.
...
BUILT_AGAINST_EXECUTOR_SCHEMA = 2
BUILT_AGAINST_PRODUCTION_SCHEMA = 2
