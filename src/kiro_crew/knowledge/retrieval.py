...
class HybridRetriever:
    ...
    def _context_for_managed_candidate(self, ctx, principal, resource):
        ...
        try:
            ...
            try:
                return self.binding_resolver.resolve(
                    principal, resource.provider, resource.account)
            except Exception:
                logger.warning(
                    "Knowledge ACL binding_resolver raised for provider=%s; "
                    "denying (fail-closed)", resource.provider, exc_info=True)
                return None
        except Exception:
            logger.warning(
    ...
    def visible_item_ids(self, item_ids, *, access_context=None,
                         query_principal=None) -> set:
