...
class SourceRow:
    ...
    * ``item_type``    -- optional item type label (default 'document').
    ...
    def acl_hash(self) -> str:
        """A stable digest of this row's GRANT facts (subjects, tenant,
        resource_ref) -- everything that decides who may see the row, but NOT its
        text. The pipeline stores it beside the text content_hash so a change
        that touches ONLY permissions (same text, new subjects/tenant/ref) is
        still detected and re-granted, instead of being skipped by the
        text-hash shortcut and leaving an obsolete grant in force."""
        ref = self.resource_ref
        ref_key = ref.to_json() if ref is not None and hasattr(ref, "to_json") else None
        payload = json.dumps(
            {
                "subjects": sorted(str(s) for s in self.subjects),
                "tenant": self.tenant,
                "resource_ref": ref_key,
            },
            sort_keys=True,
        )
...
class RowsIngestOutcome:
    ...
    rows. ``deleted_keys`` are row keys removed because a FULL snapshot does not
    list them (never populated for an incremental round).
    """
