    ...
    for dirpath, dirnames, filenames in os.walk(root, onerror=walk_errors.append):
        ...
        for dname in list(dirnames):
            ...
            if stat.S_ISLNK(mode):
                # os.walk(followlinks=False) lists a dir symlink but does not
                # descend; classify it as a link so a retargeted symlink
                # changes the fingerprint, and keep the walk out of it.
                dirnames.remove(dname)
                try:
...
def _write_provenance_marker(dest_dir: Path, fingerprint: str) -> None:
    ...
    """
    try:
        atomic_write(dest_dir / _PROVENANCE_MARKER, fingerprint + "\n")
    except OSError:
        logger.warning(
...
def _finalize_user_backup(claim: Path, dest_dir: Path) -> Path | None:
    ...
    Follows the naming and collision behavior of the ``SKILL.md.pre-relocation``
    ...
    A claim that is itself a link or junction is never entered: renaming it
    moved only the link, and touching ``claim / "SKILL.md"`` would follow the
    link and rename a file inside its TARGET tree, outside the skills
    directory. Links go to a dot-prefixed quarantine name instead (hidden from
    discovery without dereferencing anything).

    For a real directory, the backup's ``SKILL.md`` is renamed to
    ``SKILL.md.user-backup`` — the same "preserved on disk, no longer loaded"
    contract as the relocation quarantine — because the backup directory is
    not dot-prefixed and would otherwise be discovered as a live skill whose
    triggers shadow the builtin that replaced it.
    """
    linked = is_link_or_junction(claim)
    stem = f".{dest_dir.name}.user-backup" if linked else f"{dest_dir.name}.user-backup"
    ...
    if linked:
        return backup
    skill_md = backup / "SKILL.md"
    hidden = skill_md.with_name("SKILL.md.user-backup")
    counter = 2
    while os.path.lexists(hidden):
        hidden = skill_md.with_name(f"SKILL.md.user-backup.{counter}")
        counter += 1
    try:
        # lexists, not is_file: whatever occupies the SKILL.md name (a FIFO, a
        # symlink, a regular file) must stop being discoverable — the rename
        # neither opens nor follows it, so deactivating a special file is
        # exactly as safe as deactivating a regular one.
        if os.path.lexists(skill_md):
            os.replace(skill_md, hidden)
    except OSError:
        # The backup must not stay discoverable as a live skill shadowing the
        # builtin that replaced it: when the SKILL.md rename fails (e.g. a
        # non-writable tree), hide the whole backup under a dot-prefixed
        # sibling instead.
        dotted = backup.with_name(f".{backup.name}")
        counter = 2
        while os.path.lexists(dotted):
            dotted = backup.with_name(f".{backup.name}.{counter}")
            counter += 1
        try:
            os.replace(backup, dotted)
            return dotted
        except OSError:
            logger.warning(
                "could not deactivate or hide quarantine %s; it stays "
                "discoverable until reconciled", backup, exc_info=True,
            )
    return backup
        ...
        for name, src_file in _iter_skill_files(src_root):
            ...
            if dest_dir.exists() or is_link_or_junction(dest_dir):
                ...
                if claim is None:
                    continue
                if (
                    not is_link_or_junction(claim)
                    and (
                        _verified_unchanged_fingerprint(claim, src_dir) is not None
                        or not _tree_has_content(claim)
                    )
                ):
                    try:
                        shutil.rmtree(claim)
                    except OSError:
                        # A half-deletable tree must neither crash startup nor
                        # stay hidden at the claim path: put whatever remains
                        # back at its own name and retry next run.
                        logger.warning(
                            "could not remove verified skill copy %s; restoring",
                            claim, exc_info=True,
                        )
                        try:
                            os.replace(claim, dest_dir)
                        except OSError:
                            logger.warning(
                                "could not restore %s from claim %s; data "
                                "preserved there", dest_dir, claim, exc_info=True,
                            )
                        continue
                else:
    ...
    if base.exists():
        for name in stale_builtins:
            ...
            if claim is None:
                continue
            removed = False
            if _verified_unchanged_fingerprint(claim, None) is not None:
                try:
                    shutil.rmtree(claim)
                    removed = True
                    ...
                    logger.warning(
                        "could not remove stale builtin skill %s; restoring",
                        claim, exc_info=True,
                    )
            if not removed:
                # Diverged since the marker was recorded (user data), or the
                # removal itself failed: restore the tree to its original
                # name; on failure it stays hidden but intact at the claim
                # path.
                try:
