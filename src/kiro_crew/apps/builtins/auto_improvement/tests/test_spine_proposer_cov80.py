...
from kiro_crew.apps.builtins.auto_improvement.spine.contracts import Proposal
...
class TestWorktreeLifecycle:
    def test_a_new_worktree_clears_any_orphan_before_adding(
        ...
        calls: list[list[str]] = []
        monkeypatch.setattr(P, "_git", lambda args, cwd: calls.append(list(args)) or _ok())
...
class TestAgentAuthoringEscalation:
    ...
    def test_a_bug_candidate_with_no_seed_is_authored_by_the_agent(
        ...
        runner = object()
        profile = FakeProfile(produced=False)
        ...
        out = _proposer(tmp_path, agent_runner=runner).propose_one(
            profile=profile,
            candidate=FakeCandidate(kind="bug"),
            base_sha="base0",
            cycle=2,
            tier="wide",
        )
    ...
    def test_a_perf_candidate_routes_to_the_perf_author_with_no_hint(
        ...
        out = _proposer(tmp_path, agent_runner=object()).propose_one(
            profile=FakeProfile(produced=False),
            candidate=FakeCandidate(kind="perf"),
            base_sha="base0",
            cycle=2,
            tier="deep",
        )
...
class TestFanOutStopCheck:
    def test_a_stop_request_aborts_before_the_next_agent_pass(
        ...
        monkeypatch.setattr(
            P.Proposer,
            "propose_one",
            lambda self, *, profile, candidate, base_sha, cycle, tier: seen.append(
                f"{tier}:{candidate.target}"
            )
            or Proposal(
        ...
        cands = [FakeCandidate(target=f"src/m{i}.py::f") for i in range(4)]
        stop_after = {"n": 2}
    ...
    def test_without_a_stop_check_wide_and_deep_slices_stay_disjoint(
        ...
        cands = [FakeCandidate(target=f"src/m{i}.py::f") for i in range(3)]

        _proposer(tmp_path, wide=2, deep=1).fan_out(
            profile=FakeProfile(), candidates=cands, base_sha="base0", cycle=1
        )
...
class TestProposeErrors:
    def test_a_raising_profile_is_recorded_as_an_error_not_a_no_defect(
        ...
        out = _proposer(tmp_path).propose_one(
            profile=FakeProfile(raises=RuntimeError("boom")),
            candidate=FakeCandidate(),
            base_sha="base0",
            cycle=1,
            tier="wide",
