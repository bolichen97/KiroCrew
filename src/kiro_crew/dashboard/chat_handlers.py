        ...
        try:
            ...
            if agent_name:
                ...
                if not is_project_agent:
                    ...
                    elif ws_name != "default":
                        # Only a RESOLVED, NAMED workspace may retarget the
                        # project. `_workspace_name_for_dir` answers the literal
                        # "default" both for an agent bound to no workspace and
                        # for one naming a workspace absent from the config, so
                        # without this gate picking either kind discards the
                        # directory the user chose and runs the next turn's tools
