...
class TestRecoveryPutsBackWhatTheSavePreserved:
    def test_a_link_survives_the_whole_save_then_restore_round_trip(
        self, tmp_path: Path
    ) -> None:
        ...
        assert (live / "sub" / "pointer.md").is_symlink(), (
            "the link was dropped on the way back, after the live copy was already gone"
        )
...
class TestAnEmptyDatabaseIsRefused:
    ...
    def test_the_size_check_precedes_the_open(self) -> None:
        ...
        assert src.index("st_size == 0") < src.index("sqlite3.connect("), (
