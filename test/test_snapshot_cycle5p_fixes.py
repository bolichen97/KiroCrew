...
def _write_token(tmp_home: Path) -> Path:
    ...
    token.write_text(
        json.dumps({"account": ACCOUNT, "region": REGION}), encoding="utf-8"
    )
    return token
...
class TestOneAuthorizationApprovesOneDestination:
    ...
    def test_the_loser_is_told_it_was_consumed_not_that_the_json_is_broken(
        ...
        assert "JSON" not in message, (
            "an absent token was reported as a malformed one: " + message
        )
    ...
    def test_the_lock_is_actually_held_while_the_token_is_consumed(
        ...
        def probing(*args, **kw):
            ...
            try:
                probe["free"] = remote.platform_compat.try_acquire_lock(
                    fd, exclusive=True
                )
                if probe["free"]:
    ...
    def test_validation_and_deletion_run_inside_the_lock(self) -> None:
        ...
        assert src.index("_consume_verified_authorization(") > held, (
