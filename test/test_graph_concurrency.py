...
def _make_execute_dispatch(*, host: str, vault, gate, store, handle, descriptor):
    ...
    path -- no test-double capture. Custody is W01's ``gate`` + live ``store``
    (PRODUCTION_SCHEMA_VERSION 4); every hop goes through W01's execute.
    """
...
def _compose(tmp_path: Path):
    """Compose the REAL W01 custody: an encrypted vault, a live BindingStore, and
    a BindingCustodyGate for the one binding under test.

    Not stubs -- the vault is an on-disk AES-256-GCM store, the store is L04's own
    on-disk file (uniqueness domain, cross-process lock, generation fence), and
    the gate is the product's own. What the send path proves about the live-store
    fence is therefore a property of the real store, not a fake.
    """
    ...
    # record (never a slug-collapsed name), and create_binding stamped that ref.
    vault.set_sync(binding["secret_ref"]["name"], "sp-live-token")
    ...
    # The gate holds custody for THIS binding, keyed on the trusted view's
    # fingerprint -- it selects no secret and derives no vault name.
    gate = BindingCustodyGate(binding=binding, binding_fingerprint=view.binding_fingerprint)
    # A live, on-disk L04 store holding the binding: this is the trusted source
    # the transport's select_secret fences against and reads the ref from.
    store = BindingStore(tmp_path / "connections" / "control_plane_bindings.json")
    store.insert(
        binding,
        deployment_id="deployment://test/sharepoint/0",
...
def test_full_sequence_through_executor_applies_after_safe_retry(
    ...
    vault, gate, store, handle = _compose(tmp_path)
