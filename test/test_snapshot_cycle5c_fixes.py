...
from kiro_crew import snapshot_remote
...
class TestAFailedTreeReplacementPutsTheTreeBack:
    ...
    def test_the_rollback_directory_is_kept_when_recovery_itself_fails(
        ...
        # Recovery restores with the link-preserving copy, so that is where a failure has
        # to be injected -- patching the outbound one no longer reaches this path.
        monkeypatch.setattr(snap, "_copytree_rollback", refuse)
    ...
    def test_a_failed_rollback_SAVE_leaves_live_data_untouched(
        ...
        real = snap._copytree_rollback

        def fail_the_save(src, dst, **kw):
            # The SAVE direction: copying the live tree INTO the rollback directory. It
            # has its own copy helper because a rollback set must reproduce links the
            # outbound bundle deliberately drops.
            if "pre-restore-" in str(dst):
                raise OSError("Input/output error")
            return real(src, dst, **kw)

        monkeypatch.setattr(snap, "_copytree_rollback", fail_the_save)
        rc = snap.restore_main([str(bundle), "--mode", "replace", "--force"])
        assert rc == 1
        out = capsys.readouterr().out
    ...
    def test_every_tree_save_precedes_the_mutation_phase(self):
        ...
        for saving in ("backup / dirname)", 'backup / "skills")'):
            for copier in ("_copytree_rollback", "_copytree_safe"):
        ...
        assert setup.index("_copytree_rollback(d, backup / dirname)") < setup.index(
...
class TestAnUnrestorableBundleIsNeverUploaded:
    def test_a_bundle_over_the_download_bound_is_not_published(
        ...
        monkeypatch.setattr(
            snapshot_remote, "upload", lambda *a, **k: uploaded.append(a) or "s3://x/y"
        )
        monkeypatch.setattr(
            snapshot_remote, "load_destination", lambda *a, **k: _FakeDest()
        )
