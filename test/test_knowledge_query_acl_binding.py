...
def _managed(store, title, content, *, provider, account, subjects, tenant, ref_locator=None):
    ...
    ref = ProviderResourceRef(
        provider=provider, account=account, resource_id=title, locator=ref_locator or {}
    )
    store.set_item_acl(
        item, subjects, tenant=tenant, managed=True, fresh_as_of=time.time(), resource_ref=ref
    )
    return item
...
def test_managed_without_resource_ref_denied_under_resolver(store):
    ...
    store.set_item_acl(
        item, ["sp-alice"], tenant="ms-A", managed=True, fresh_as_of=time.time()
    )  # no resource_ref
...
def test_bridge_maps_accessgrant_into_a_working_gate(store):
    ...
    inner = _GrantResolver(
        {("sharepoint", "tenant-A"): _AccessGrant(subject="sp-alice", tenant="ms-A")}
    )
...
def test_bridge_wrong_account_grant_denies(store):
    ...
    inner = _GrantResolver(
        {("sharepoint", "tenant-B"): _AccessGrant(subject="sp-alice", tenant="ms-A")}
    )
    bridged = bridge_binding_resolver(inner)
    r = HybridRetriever(store, revalidator=_fresh_hook(), binding_resolver=bridged)
    r._keyword_search = lambda *a, **k: [(item, 1)]
...
def test_bridge_group_from_grant_satisfies_subject_test(store):
    ...
    inner = _GrantResolver(
        {
            ("sharepoint", "tenant-A"): _AccessGrant(
                subject="sp-alice", tenant="ms-A", groups=frozenset({"sp-team"})
            )
        }
    )
    bridged = bridge_binding_resolver(inner)
...
# ── forward-compat: the bridge prefers W01's resolve_ref(principal, ref) ─────
# W01 is moving to resolve_ref(principal, ref) over the whole four-field
# ProviderResourceRef (it carries the endpoint the (provider, account) pair
# lacks). The bridge must PREFER it when the inner exposes it and a ref is given,
# and fall back to the older resolve(principal, provider, account) otherwise.


class _RefOnlyResolver:
    """A W01-shaped resolver exposing ONLY resolve_ref(principal, ref)."""

    def __init__(self):
        self.seen = []

    def resolve_ref(self, principal, ref):
        self.seen.append(ref)
        # Grant only when the ref's endpoint matches (proves the whole ref, incl.
        # the endpoint, reaches the resolver -- not just provider/account).
        loc = getattr(ref, "locator", {}) or {}
        if ref.provider == "sharepoint" and loc.get("endpoint") == "host-1":
            return AccessContext(subject="sp-alice", tenant="ms-A")
        return None


def test_bridge_prefers_resolve_ref_and_passes_the_full_ref(store):
    # A resource_ref whose locator carries the endpoint the resolver checks.
    ref = ProviderResourceRef(
        provider="sharepoint", account="tenant-A", resource_id="Doc", locator={"endpoint": "host-1"}
    )
    src2 = store.add_source("sp2", "sharepoint", "sharepoint://Doc2")
    item2 = store.add_item("Doc2", "omega content", "doc", source_id=src2)
    store.set_item_acl(
        item2, ["sp-alice"], tenant="ms-A", managed=True, fresh_as_of=0.0, resource_ref=ref
    )

    inner = _RefOnlyResolver()
    bridged = bridge_binding_resolver(inner)
    r = HybridRetriever(store, revalidator=_fresh_hook(), binding_resolver=bridged)
    r._keyword_search = lambda *a, **k: [(item2, 1)]
    res = r.search("omega", limit=10, query_principal=QueryPrincipal("alice"))
    assert item2 in _ids(res)
    # The WHOLE ref (with endpoint) reached resolve_ref, not just provider/account.
    assert inner.seen and getattr(inner.seen[0], "locator", {}).get("endpoint") == "host-1"


def test_bridge_resolve_ref_wrong_endpoint_denies(store):
    ref = ProviderResourceRef(
        provider="sharepoint", account="tenant-A", resource_id="Doc", locator={"endpoint": "host-2"}
    )
    src = store.add_source("sp3", "sharepoint", "sharepoint://Doc3")
    item = store.add_item("Doc3", "omega content", "doc", source_id=src)
    store.set_item_acl(
        item, ["sp-alice"], tenant="ms-A", managed=True, fresh_as_of=0.0, resource_ref=ref
    )
    bridged = bridge_binding_resolver(_RefOnlyResolver())
    r = HybridRetriever(store, revalidator=_fresh_hook(), binding_resolver=bridged)
    r._keyword_search = lambda *a, **k: [(item, 1)]
    # host-2 not granted by the resolver -> denied (proves endpoint discriminates).
    assert r.search("omega", limit=10, query_principal=QueryPrincipal("alice")) == []


def test_bridge_falls_back_to_resolve_when_no_resolve_ref(store):
    # An inner with only the old resolve() still works (fallback path).
    item = _managed(
        store,
        "Doc",
        "omega content",
    ...
    assert not hasattr(inner, "resolve_ref")
    bridged = bridge_binding_resolver(inner)
    r = HybridRetriever(store, revalidator=_fresh_hook(), binding_resolver=bridged)
    r._keyword_search = lambda *a, **k: [(item, 1)]
    assert item in _ids(r.search("omega", limit=10, query_principal=QueryPrincipal("alice")))
