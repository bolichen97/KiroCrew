...
class TestPromptResolution:
    ...
    def test_proc_environ_prompt_is_refused(self, tmp_path):
        # /proc/self/environ would leak the gateway's own environment; it is a
        # pseudo-filesystem is_sensitive_path does not cover, so the explicit
        # pseudo-fs guard is what stops it.
        with pytest.raises(KasAgentTranslationError, match="not an allowed location"):
            resolve_prompt(
