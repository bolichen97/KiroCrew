...
def test_bundle_import_omitted_managed_defaults_to_managed(store):
    # An item_acl entry omitting `managed` must import as managed=1 (fail-closed),
    # matching the documented behavior -- an untrusted bundle cannot un-gate an
    # item by leaving the flag out.
    store.db.execute(
        "INSERT OR IGNORE INTO items (id, title, content, item_type, created_at, updated_at) "
        "VALUES ('it-x','t','c','document','2020-01-01','2020-01-01')"
    )
    store.db.commit()
    bundle = {
        "sources": [],
