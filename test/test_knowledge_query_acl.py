...
def test_policy_managed_item_denied_for_bypass():
    ...
    pol = SubjectTenantAclPolicy()
    g = ItemGrant(
        subjects=frozenset({PUBLIC_SUBJECT}), tenant="t", managed=True, fresh_as_of=_fresh()
    )
...
def test_policy_managed_stale_denied_without_revalidation():
    ...
    fresh = ItemGrant(
        subjects=frozenset({"alice"}), tenant="t", managed=True, fresh_as_of=_fresh(-10)
    )
...
def test_revalidation_request_contract_fields():
    ...
    bare = RevalidationRequest(
        ctx=AccessContext(subject="a", tenant="t"),
        item_id="x",
...
def test_mixed_library_local_readable_managed_gated(store):
    ...
    local_item = store.add_item("Vault Note", "alpha local content", "doc", source_id=local_src)
    cloud_item = store.add_item("SP Doc", "alpha cloud content", "doc", source_id=cloud_src)
    # cloud item's grant, freshly revalidated, for alice@acme
    store.set_item_acl(cloud_item, ["alice"], tenant="acme", managed=True, fresh_as_of=_fresh())
...
def test_managed_item_no_grant_denied_to_everyone(store):
    ...
    assert (
        r.search("gamma", limit=10, access_context=AccessContext(subject="alice", tenant="acme"))
        == []
    )
...
def test_unknown_new_cloud_type_is_managed_and_gated(store):
    ...
    assert (
        store.db.execute("SELECT trust_class FROM sources WHERE id = ?", (src,)).fetchone()[0]
        == TRUST_MANAGED
    )
    ...
    assert (
        r.search("zeta", limit=10, access_context=AccessContext(subject="alice", tenant="acme"))
        == []
    )
...
def test_managed_source_cannot_be_relabelled_local_via_missing_flag(store):
    ...
    rr = _retriever(
        store, kw=[(item, 1)], revalidator=_StubRevalidator(RevalidationOutcome.REVOKED)
    )
...
def test_sourceless_item_is_managed_failclosed(store):
    ...
    assert (
        r.search("epsilon", limit=10, access_context=AccessContext(subject="alice", tenant="acme"))
        == []
    )
...
def test_two_identities_cloud_A_sees_B_does_not(store):
    ...
    assert item in _ids(
        r.search(
            "budget alpha", limit=5, access_context=AccessContext(subject="alice", tenant="acme")
        )
    )
    assert (
        r.search(
            "budget alpha", limit=5, access_context=AccessContext(subject="bob", tenant="acme")
        )
        == []
    )
...
def test_same_email_different_tenant_is_different_identity(store):
    ...
    store.set_item_acl(item, ["alice@x.com"], tenant="tenant-1", managed=True, fresh_as_of=_fresh())
    r = _retriever(store, kw=[(item, 1)])
    assert item in _ids(
        r.search(
            "tenant",
...
def test_public_managed_item_visible_to_tenant_subjects_when_fresh(store):
    ...
    for who in ("alice", "bob"):
        assert item in _ids(
            r.search("content", limit=5, access_context=AccessContext(subject=who, tenant="acme"))
        )
    # cross-tenant public
    store.set_item_acl(
        item, [PUBLIC_SUBJECT], tenant=PUBLIC_TENANT, managed=True, fresh_as_of=_fresh()
    )
    r2 = _retriever(store, kw=[(item, 1)])
    assert item in _ids(
        r2.search("content", limit=5, access_context=AccessContext(subject="z", tenant="other"))
    )
...
def test_unreadable_grant_row_fails_closed(store):
    ...
    r = _retriever(store, kw=[(item, 1)])
    assert (
        r.search("content", limit=5, access_context=AccessContext(subject="alice", tenant="acme"))
        == []
    )
...
def test_forged_local_stamp_refused_for_non_local_type(store):
    ...
    assert (
        store.db.execute("SELECT trust_class FROM sources WHERE id = ?", (sp,)).fetchone()[0]
        == TRUST_MANAGED
    )
    # Forge local on an unknown type.
    unk = store.add_source("U", "brand_new_2027", "u://x", trust_class=TRUST_LOCAL)
    assert (
        store.db.execute("SELECT trust_class FROM sources WHERE id = ?", (unk,)).fetchone()[0]
        == TRUST_MANAGED
    )
    # A genuine local creator type IS honoured.
    lf = store.add_source("V", "local_folder", "file:///v", trust_class=TRUST_LOCAL)
    assert (
        store.db.execute("SELECT trust_class FROM sources WHERE id = ?", (lf,)).fetchone()[0]
        == TRUST_LOCAL
    )
    ...
    assert (
        store.db.execute("SELECT trust_class FROM sources WHERE id = ?", (lf2,)).fetchone()[0]
        == TRUST_MANAGED
    )
...
def test_managed_grant_survives_reassignment_into_local_source(store):
    ...
    assert (
        rr.search("theta", limit=10, access_context=AccessContext(subject="alice", tenant="acme"))
        == []
    )
