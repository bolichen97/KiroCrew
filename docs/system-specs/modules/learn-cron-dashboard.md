...
- **`cron_store_lock` (module-level)**: the store's advisory lock has ONE implementation, `cron.cron_store_lock(store_dir, *, timeout, poll)`; `CronService._file_lock` delegates to it after its loop-safety guard, and the Agent templates delete guard (`handlers/agent_templates.py`) takes it directly, off-loop, around its dispatch-reference walk and the template file's rename — so a schedule cannot be saved against a template between the guard's "nothing dispatches this" and the file going. Its `CronStoreBusy` surfaces there as `503 schedule_store_busy` (retryable, nothing unlinked). Innermost in that caller's lock order; the mutators take only this lock, so the order cannot invert.
...
- `folder_repository.py` — chat-folder load, serialized read-modify-write, rollback,
  ...
  across an awaitable, read-only section that may hop off the loop — how the
  agent-template delete guard excludes a concurrent folder pin without doing
  file work on the loop), and breadcrumb traversal. `DashboardState` keeps
  its existing folder methods as compatibility facades; request handlers continue
  to use `mutate_folders()` / `read_folders()` / `hold_folders()` and never
...
- **Source-provider mutation ownership**: GitHub/GitLab source reads may use a signed machine-local bootstrap subject (`local-app` / `local-startup`) before an owner is configured, but every provider mutation remains owner-only. When one of those signed local dashboard sessions reaches a mutation with no configured owner, the gateway still returns `403` but labels the already-denied response with `code: "owner_not_configured"`; the Changes panel, review threads, and Code Review Sage replace the server's English fallback with localized guidance and link to Settings → Messaging Channels → Slack. App tokens, unauthenticated requests, and non-local subjects retain the generic forbidden body so the response does not disclose the install's owner state.
...
**Agents**: GET `/api/agents` (Kiro Crew agent roster ordered **most-used-first** — config agents plus, scoped to the requesting session's project directory via `X-Session-Key`, that project's `.kiro/agents` discoveries, each row tagged `scope: "global" | "project"`; a name in both scopes lists once as the alias, since dispatch resolves aliases first; ordering reorders by `ConversationLog.agent_usage()` (turn count, then recency), falling back gracefully to config-insertion order on any failure so the dropdown never breaks or drops agents), GET `/api/agents/installed` (list all kiro-cli agents from `~/.kiro/agents/` — deliberately global-only, because every consumer is an agent CRUD/editor surface whose "Set as default" persists the selected name into `cfg.agents`, where a project-only name would not resolve; project-scope discovery instead reaches the dispatch surfaces — per-turn resolution, spawn validation, and Slack — with `scope` on each entry reporting which scope won there; `package` field extracted from filename), GET/PATCH/DELETE `/api/agents/detail/{name}` (full agent config JSON; PATCH writes `model` and `skills` for any user-level spec — the crew pane's private-copy edits — and, for the Agent templates tab, the definition keys `description`, `prompt`, `tools`, `allowedTools`, which are shape-checked (`400 invalid_definition`), refused on a package, runtime, markdown or private-copy spec (`409 template_read_only`, `reason` names which) and refused when more than one file claims the name (`409 ambiguous_template_name`); DELETE removes a template the user owns, `409 template_read_only` for the same four, `409 ambiguous_template_name` when more than one file reaches the name (by declared `name` or stem — the first hit is never unlinked), `503 schedule_store_unreadable` when `crons.json` is present but unreadable (nothing unlinked; a repaired store brings its jobs back), `409 template_referenced` with `references: [{kind: crew|default|schedule|folder|webhook|private_copy, id, label}]` (ids and labels through `_roster_mask`) while anything still names it; a row whose `filename` is not a plain basename resolving to a regular file directly under the agents directory is `404 template_not_found`, never unlinked — `folder` is a chat folder whose `default_agent` pins the template, `webhook` a webhook token whose `agent` does; DELETE and PATCH are owner-only — see Agent Config), GET `/api/agents/templates` (the templates tab's roster: every global discovery row with its package-controlled strings — description, model, source, package, lineage, skills, MCP server names, reference ids and labels — through `_roster_mask`, a row whose name or filename would be masked omitted, plus `read_only` — `null` or the reason — and `used_by`, the same reference list the delete guard evaluates), POST `/api/agents/templates` (`{name, description?, from?}` creates `~/.kiro/agents/<name>.json`, blank or as a lineage-free copy of `from`; `400 invalid_template_name` / `template_name_reserved`, `404 template_not_found`, `409 name_taken` / `name_bound` (a binding in `config.json` or only in the `config.local.json` overlay, both checked under their own locks) / `ambiguous_template_name`; enrols nothing)
...
**Terminal tab names** (`BottomTerminalPanel.tsx`, `useBottomTerminal.ts`): docked and popped-out terminal tabs support inline rename by double-click, F2 on the focused tab, or the tab context menu's Rename action. Enter or blur saves; Escape cancels; composition keystrokes do neither. The close control is withdrawn while the editor is open, so leaving the field cannot both save a name and kill its shell. The optional `TermTab.name` is a trimmed, 120-code-point browser-local label. Each explicit rename writes only `mc-terminal-name:<terminal-id>`, not the whole `mc-bottom-terminal` layout. Separate tab coordinates let renames commute with layout writes and with renames of other tabs; two renames of the same tab are last-write-wins. Layout loads and writes overlay those coordinates, and label storage events update only the corresponding in-memory label without repersisting a layout. Event handlers read current storage rather than a queued event's older value. Embedded layout names remain a backward-compatible fallback, including on uiPrefs restore; an empty-string coordinate explicitly resets that fallback to the automatic title. Removing a tab (including the confirming hydrate probe) cleans its coordinate after layout persistence; a rename checks persisted membership before writing and cleans again afterwards to handle a concurrent close. Reload reclaims orphan coordinates only when layout membership can be read, preserving labels on malformed/unreadable storage. Persistence remains best-effort under denied storage or exhausted quota. `uiPrefs` projects current labels into its existing layout backup on read, including fingerprints and resets, without writing the projection back to localStorage or adding per-session keys to its allowlist. This does not make unrelated concurrent layout mutations atomic. It overrides, but does not replace, the shell's live title; an empty edit or Use automatic name removes the override. Renaming never changes the PTY id, cwd, socket or mounted shell. F2 and context-menu rename leave the active tab unchanged so revealing an inactive terminal cannot steal the editor's focus. Pointer clicks select immediately so terminal autofocus can accept the first typed character. Because `click` precedes `dblclick`, double-click rename captures the prior active tab on the first click and synchronously restores that still-valid tab before mounting the editor, provided no intervening selection occurred. The transient first-click selection is unavoidable without delaying terminal input; the editor opens over the restored terminal and the final active tab is unchanged. Keyboard and assistive selection are immediate too. Selection speaks in fill alone (the elevated pill), and keyboard focus is split by selection state so the strip never shows two accent marks: the selected pill keeps the global two-pixel accent outline (ring and pill are one element, so the ring can only read as focus on the selection), while an inactive chip paints the same two-pixel, two-pixel-offset outline in neutral `--muted` (`focus-visible:outline-2 focus-visible:outline-muted focus-visible:outline-offset-2`, which outspecify the bare global `:focus-visible` rule). Focus returns to an inactive chip after every rename of a tab that is not selected, which is where an accent ring beside the pill reads as a second selection at any strength — so the cue differs in colour, not merely strength, the same rule the swatch hover cue follows; `--muted` is the lightest neutral token clearing the 3:1 non-text floor on `--bg` in both default themes (measured 5.66:1 dark, 6.78:1 light). The context menu's Rename item shows `F2` as a trailing muted badge (`aria-hidden`, `data-i18n-opaque`) and declares it on its own `aria-keyshortcuts`, in the same shape as the nav rail's chord badges; the chip keeps its `aria-keyshortcuts="F2"` too, and the item's accessible name stays "Rename". While a name is being edited the chip sheds its pill and the shared filled `Input` is the only box, and a visible helper (`terminalTab.edit_hint`: Enter saves, Escape cancels, empty restores the automatic name) is overlaid beneath the strip and linked to the editor by `aria-describedby`; it is overlaid rather than stacked so the shells are never refit or resized. The scroll strip reserves four pixels on every edge for the two-pixel outline and its two-pixel offset in either colour. Keyboard focus reveals the tab within that inset, including after the rename input shrinks back to its label. `website/scripts/capture-terminal-rename.mjs` (gateway-free, built SPA) measures both outlines — colour token, that the neutral one differs from `--accent` and `--ring`, extent, containment and contrast against `--bg` in both themes — asserts the F2 badge, and writes the context-menu frame and a save/cancel recording as evidence.

A dropped terminal-name write remains authoritative in that document's live state,
including across later layout writes and foreign layout events. A volatile per-tab
entry holds the edit and its persisted fallback; layout serialization and uiPrefs
exclude that volatile edit. A successful same-window rename or an actual foreign
...
**Streaming rendering** — Two-layer architecture: (1) `useBlockAssembler` hook parses raw text into `ContentBlock[]` via state machine tracking `paragraph`/`fenced_code` states; (2) `BlockRenderer` dispatches each block to specialized renderers (`MarkdownBlock`, `CodeBlock`, `DiffBlock`, `MermaidBlock`, `ExcalidrawBlock`). During streaming (`streaming=true`), unclosed code fences produce blocks with `complete: false` that render as provisional views. Diagram blocks are held at the placeholder until the fence closes — a half-streamed Mermaid graph or Excalidraw scene is not yet valid, so drawing it would only flash the fallback. A closed Mermaid fence is then drawn only once both preconditions of a trustworthy label measurement hold: its host has a box (mermaid measures labels with `getBoundingClientRect()` on a scratch element under `document.body`, and inside a `display:none` pane every rect is 0, so the block probes `getClientRects()` and waits on a `ResizeObserver` for the box to come back) and `document.fonts.ready` has settled (the body face is swap-loaded, so a label measured in the fallback face and painted in the loaded one is clipped at its right edge), the wait on `ready` capped at 2.5 s (`MERMAID_FONTS_READY_CAP_MS`) because a font file whose packets are dropped keeps the set pending for the browser's network timeout, so past the cap the diagram is drawn in the fallback face and the late `loadingdone` redraw below corrects it. Both are watched for the whole of `mermaid.render()`; a box lost mid-render discards that SVG and waits for the box again, and a `loadingdone` mid-render or a load still pending at the end (unless the gate hit the cap, in which case that load is the one it gave up on and the late redraw covers it) discards it for exactly ONE more attempt, bounded so a face that keeps loading cannot redraw the diagram forever. The font watch outlives the draw: when the swap-loaded stylesheet is itself the late arrival, no face is pending when `ready` is read and the diagram is drawn in the fallback face, so the first `loadingdone` after the draw redraws it, once, and the watch is spent (released on that redraw or on unmount). Where `document.fonts` is absent the fonts gate is a no-op. On completion (`streaming=false`), full reparse from content produces clean final blocks. `ChatMessage.rawText` preserves the original unprocessed text as source of truth for reparse.
...
The in-UI template delete returned with the Agent templates tab
(`handlers/agent_templates.py`, `DELETE /api/agents/detail/{name}`), rebuilt
from the handler this repo's history retired with the old standalone page: the
same lock discipline (config lock, then the spec lock around the unlink), the
same alias set (request name, declared name, filename stem) and a 409
reference guard — widened from crews and the default agent to schedules
(`crons.json`, read as dispatch reads it: a multi-entry `agent_sequence`
over a dormant `agent_id`, else the captured `execution_context.template_id`
— the template a schedule created from a template chat names, with `agent_id`
empty — falling back to `agent_id` for a legacy record; script and command
jobs skipped), chat folders
(`default_agent` in the folder store — the check and the unlink run off-loop
inside that store's hold,
`state.hold_folders`, so a concurrent pin cannot slip between them), webhook
tokens (`agent` in the token store) and private copies (`forked_from` in the
fork sidecar); the check reads the effective config under the `config.json`
and `config.local.json` advisory locks (cross-process, nested, writing
nothing), and answers with the reference LIST rather than prose so the tab can
offer a jump to each. A package, runtime, markdown or private-copy spec is
refused as `template_read_only` before the guard runs. Deleting on disk still
works as documented in `src/kiro_crew/docs/agents.md`; a crew bound to a name
