...
- **A path can still collide with a command-oriented built-in rule.** Encoding whitespace defuses the multi-token rules (`rm -rf /`, `terraform destroy`, `DROP TABLE`), but a whitespace-free pattern can match a legitimate path: `mkfs.*` against `path=/srv/mkfs-tests`, or `.*python.*botocore.*credentials.*` against a real virtualenv path. The built-in rules themselves are unchanged, but synthesizing a path target is what newly exposes them to path text, so this collision is introduced here rather than pre-existing. It is an over-block in the fail-safe direction, and the offending built-in is disable-able by rule id from Settings → Security.
