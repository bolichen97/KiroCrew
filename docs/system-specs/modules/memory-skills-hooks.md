...
- **V1 cap**: `_DEFAULT_EPISODIC_MAX` = 10,000 active entries. For V1, `_enforce_episodic_cap()` tombstones `ORDER BY importance ASC, created_at ASC` (lowest-importance oldest first) on write once the count reaches the cap. V2 bypasses capacity eviction and retains the stored episodes.
...
**Probe temp**: the probed child gets a managed temp directory under `<data home>/run/mcp-tmp/probe-<id>/tmp`, carved out of the sandbox seal and exported as `TMPDIR`/`TMP`/`TEMP`, unless the spec itself declares one of those keys (matched case-insensitively). A declared temp is honoured only after `sandbox.classify_declared_temp_path` (the seal's own module) clears it; a declared `tmpdir` is re-emitted under the canonical uppercase key with the ambient siblings dropped, so the declaration governs on POSIX (where `tempfile` reads only the uppercase names) and on Windows (where the two spellings are one variable). The classification returns a cause or `None`: `sealed` when the path lies inside `<data home>/run` — judged on `realpath` of the original spelling, then on the lexical spelling, then by `(st_dev, st_ino)` identity against the sealed parent so a case, firmlink or normalization alias cannot walk past it — and `unclassifiable` when the canonical form cannot be established (`realpath(strict=True)` raises for a symlink cycle or an untraversable component; a component that does not exist yet is the ordinary case and resolves as far as it goes). Any cause refuses the declaration as a whole (`tempfile` reads `TMPDIR` before `TMP`, so a surviving sibling key would decide writability), the managed temp takes over, and one WARNING names each refused key, its path and the cause. The check runs off the event loop; if it raises, every declared key is refused under a `check-failed` cause and the WARNING names the exception (fail closed). If the managed allocation fails after a refusal, every temp key is stripped so the child falls back to its platform default rather than the refused path. **On `win32` the classification returns `None` for every path and the declared temp is honoured as declared**: Kiro Crew has no native Windows sandbox backend, so nothing seals `run/` there, a declared temp under it is writable, and resolving untrusted config text on Windows would itself open whatever it names (`realpath` and `stat` open the path there). The runtime spawn path (`spawn_backend` in `mcp_gateway/backend.py`, and the `_spawn` closure in `gatewayd.py` that signals the declared keys) honours a declared temp without this check; #9843 tracks wiring it, and until then a probe-green result does not model runtime for a spec that declares a sealed temp.
