...
- `[AVAILABLE AGENTS]` — a bounded roster of the OTHER installed agents, one `- <name>: <description>` row each, built by `_build_agent_roster_section()` and injected right after `[WORKSPACE IDENTITY]` for **every** agent (a custom orchestrator is exactly the caller that needs it) and skipped on the minimal-context path. It exists so a delegation can be routed by fit: without it the model knows agent NAMES (from `spawn_run`'s parameter description) but nothing about what any of them is for, and falls back to the host default. Names come from `subagent.visible_agent_names`, so the block inherits that helper's `_AGENT_NAME_RE` grammar filter, redaction, and reserved-conductor exclusions rather than re-deriving a fourth copy of them; the current agent is a FURTHER exclusion on top of that set. Descriptions are free text no grammar can vet, so each is flattened to one row (a multi-line description could otherwise forge a row or a header), scrubbed with `_scrub_member_payload`, redacted, then truncated — in that order, since redaction substitutes a longer marker. Bounded on THREE axes (`_ROSTER_MAX_AGENTS` = 8 rows, `_ROSTER_DESC_MAX_LEN` = 100 chars each, remainder reported as `(+N more — call spawn_list for the full roster)`) — the first two keep the section inside `_PREAMBLE_HEADROOM` on a host with a hundred agents installed, and the third, `_ROSTER_DESC_SCAN_LIMIT` (= 8 × the render bound), bounds the SCAN: the scrub chain is per-character and nothing caps the raw `description` (`spec_str` only type-checks; a spec file is read up to `hooks.MAX_FILE_BYTES` = 50 MB), so an over-limit description is pre-cut on a whitespace boundary with the straddling token dropped. Cutting on a boundary rather than blindly is what preserves the redact-then-truncate order — a credential split mid-token survives as a prefix the redaction patterns no longer match, and the whitespace collapse then pulls it inside the rendered bound. `project_dir` is threaded through `list_agents` so a project agent that shadows a user-level one is described correctly, and an unreadable agents directory degrades the block to `""` — it costs the routing hint, never the session.
...
|---|---|---|
| conduct | `_CRITICAL_RULES`, `[CURRENT DATE]`, agent identity + `[RUNTIME]`, UI language, `[WORKSPACE IDENTITY]`, `[AVAILABLE AGENTS]`, skills index | no |
...
conduct is not switchable because every member is an output contract or a capability pointer: a sub-agent without the skills index cannot discover what it can do, one without `[AVAILABLE AGENTS]` cannot discover which specialist to delegate to and re-runs the work itself, and one without `_CRITICAL_RULES` cannot format what it reports back.
