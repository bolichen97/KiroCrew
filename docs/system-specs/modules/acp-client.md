...
- `"kas"` (`ACP_BACKEND_KAS`): Kiro Agent Server as the harness, via **two paths that must not be confused**. Both are first-party, so this is not a second vendor and does not reopen the "other providers" question. **Default path (no install):** kiro-cli embeds KAS and selects it with `KIRO_CLI_KAS_ENGINE_ARG` (`--agent-engine=v3`), so the argv is just `kiro-cli acp --agent-engine=v3 --agent <name>`. Verified empirically — `kiro-cli acp --agent-engine=v3` logs `kas.server.starting {"product":"KAS (Kiro Agent Server)","version":"0.38.7"}` with zero setup, because the CLI carries `node_modules/@kiro/agent/dist/server/acp-server.js` and spawns it itself. On this path Kiro Crew is still talking to **kiro-cli**, so `_is_kiro_cli` stays true (the sandbox wrapper depends on that) and the handshake — including the date-string `protocolVersion` — is unchanged from the default backend. **Optional path (`agent.kas_path`, or `KIRO_KAS_SERVER_PATH` / `KIROCREW_KAS_ENTRY` / `KIROCREW_KAS_ROOT`):** spawns a LOCAL KAS build directly, which is the only way to run a harness build the installed CLI does not carry — the point being to A/B a KAS change without waiting for a CLI release. `_is_kas_direct` selects it; that path is a bare Node process, so `_is_kiro_cli` is false and `PROTOCOL_VERSION_KAS` applies. Resolved by `_resolve_kas_acp_bin(configured_root)` → `list[str] | None`, in order: the entry-script env vars (kiro-cli's `KIRO_KAS_SERVER_PATH` first so one export drives both tools), then `agent.kas_path`, then `KIROCREW_KAS_ROOT`. An entry override that is **not a file returns None rather than falling through** to the configured root — silently running a *different* build than the operator named is worse than failing to start. **PATH is deliberately never searched**: KAS ships no PATH-installed launcher, so a PATH hit would be an unrelated binary. `_kas_argv` mirrors kiro-cli's own launch shape, and two of its flags are load-bearing rather than cosmetic: `--experimental-wasm-modules` (the policy engine's Cedar and tree-sitter grammars are WASM modules, so without it shell parsing fails at *import*) and `--auth=acp-callback` (token refresh is host-mediated rather than read from a cached token file). Node is resolved explicitly — an extracted bundle's own `.node/bin/node` beats both `KIRO_KAS_NODE_PATH` and the host's, because the bundle's native modules are prebuilt against one Node ABI and that constraint is not a preference an operator can override. Not cached in a module global like `_claude_acp_argv_cache`, because the root is per-session config and a cache would pin the first session's build for the process lifetime. Unlike the claude seam, neither path needs handshake substitutions: KAS implements standard `session/set_mode` and `session/set_model`, its `PermissionOption`s use the ACP-spec `optionId`/`name` + `kind` shape `_build_permission_event` already reads, and it loads its own `mcp.json` rather than needing servers injected into `session/new`.
...
- `protocolVersion` in the `initialize` request: kiro-cli expects the date string `"2025-08-22"`; claude-agent-acp expects an integer (`1`, per the upstream ACP SDK schema). A **direct** KAS spawn (`PROTOCOL_VERSION_KAS = 1`) is on that same SDK, so it takes the integer — but note KAS **echoes the client's value back verbatim without validating or clamping it**, so a successful handshake there is not evidence of agreement: `1` is correct because the SDK says so, not because KAS confirmed it. The default KAS path goes through kiro-cli, so it keeps the date string.
